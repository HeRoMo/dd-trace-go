...
var _ goredis.Hook = (*datadogHook)(nil)
...
func TestSkipRaw(t *testing.T) {
	runCmds := func(t *testing.T, opts ...ClientOption) []mocktracer.Span {
		...
		client := NewClient(&goredis.Options{Addr: "127.0.0.1:6379"}, opts...)
...
func TestClientEvalSha(t *testing.T) {
	ctx := context.Background()
	opts := &goredis.Options{Addr: "127.0.0.1:6379"}
	assert := assert.New(t)
	mt := mocktracer.Start()
	defer mt.Stop()
...
func TestWrapClient(t *testing.T) {
	simpleClientOpts := &goredis.UniversalOptions{Addrs: []string{"127.0.0.1:6379"}}
	simpleClient := goredis.NewUniversalClient(simpleClientOpts)

	failoverClientOpts := &goredis.UniversalOptions{
		MasterName: "leader.redis.host",
		Addrs: []string{
			"127.0.0.1:6379",
			"127.0.0.2:6379",
		}}
	failoverClient := goredis.NewUniversalClient(failoverClientOpts)

	clusterClientOpts := &goredis.UniversalOptions{
		Addrs: []string{
			"127.0.0.1:6379",
			"127.0.0.2:6379",
		},
		DialTimeout: 1}
	clusterClient := goredis.NewUniversalClient(clusterClientOpts)
...
func TestError(t *testing.T) {
	t.Run("wrong-port", func(t *testing.T) {
		ctx := context.Background()
		opts := &goredis.Options{Addr: "127.0.0.1:6378"} // wrong port
		assert := assert.New(t)
		mt := mocktracer.Start()
		defer mt.Stop()
